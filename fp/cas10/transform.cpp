...
int main(int argc, char *argv[])
{
    ...
    std::transform(std::cbegin(items), std::cend(items), // sta transformisemo 
                   std::begin(selection_markers), // gde smestamo rezultat
                   is_selected); // transformacija

    std::copy(std::begin(selection_markers), std::end(selection_markers),
              std::ostream_iterator<bool>(std::cout, "\n")); // ispisujemo na std izlaz

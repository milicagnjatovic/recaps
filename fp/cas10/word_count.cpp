...
// g++ -o exe word_count.cpp -I ../range-v3-0.11.0/include/
// cat text.txt |./exe 
// ./exe 5 < text.txt 
...
std::string string_to_lower(const std::string &s) {
    return s | view::transform(tolower) | to<std::string>(); // to prebacuje u odgovarauci kontejner
}
...
int main(int argc, char *argv[])
{
    const int n = argc <= 1
                    ? 10
                    : atoi(argv[1]); // koliko reci brojimo
